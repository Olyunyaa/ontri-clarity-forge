...
- **Аналитика GA4 подготовлена, ждёт Measurement ID** (20.09.2026): куки-баннер переделан в Accept/Decline (EN/NL/RU), GA грузится только после согласия (`src/lib/analytics.ts`, ключ localStorage `analytics-consent`). Тот же consent-gated снипет вставлен в `/check/` и `/audit/`. В коде стоит заглушка `G-XXXXXXXXXX` (с ней GA не грузится). Когда Ольга создаст GA4-property и даст реальный ID — заменить заглушку в 3 файлах (analytics.ts, check/index.html, audit/index.html) и задеплоить.
...
- [ ] Аналитика: выбран GA4, код готов (20.09.2026) — осталось получить Measurement ID от Ольги и заменить заглушку G-XXXXXXXXXX
