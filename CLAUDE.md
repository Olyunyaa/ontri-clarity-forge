...
- **AI Automation Check** (v3, 02.08.2026) на /check/: честный однозадачный чек «Is this task worth automating?». 7 вопросов: где живёт работа (7 категорий) → что делаете (списки зависят от категории) → тест на инструкцию → частота → реальное время команды на прогон (число, не бакеты; есть «не замеряли») → похожесть прогонов → что улучшится. Шесть вердиктов правилами: Worth a pilot / Automate the edges, not the core (встречи, «сами разговоры») / Standardise first / Measure first / Not worth it yet (<40 ч/год) / Question the task («ничего конкретного»). ROI только из чисел пользователя с открытой формулой. Без LLM. Дизайн вердиктов утверждён Ольгой.
